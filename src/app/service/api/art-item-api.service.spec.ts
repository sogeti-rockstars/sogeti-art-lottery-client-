...
describe('ArtItemApiService', () => {
    ...
    it('should delete the last item of artItems', async () => {
        ...
        await lastValueFrom(service.deleteArtItem(listPreSpec![listPreSpec!.length - 1].id));
        listPostSpec = await lastValueFrom(service.getArtItems());
        expect(listPreSpec!.length).toBe(listPostSpec!.length + 1);
        expect(listPreSpec![listPreSpec!.length - 2]).toEqual(listPostSpec![listPostSpec!.length - 1]); // Next to last should be same as last
    });
    ...
    it('should delete the item with a specified randomly selected existing id', async () => {
        ...
        let rItemFromGetAll = listPreSpec![Math.floor(Math.random() * listPreSpec!.length)];
        console.log(`Deleting item with id: ${rItemFromGetAll.id}`);
        await lastValueFrom(service.deleteArtItem(rItemFromGetAll.id));
        listPostSpec = await lastValueFrom(service.getArtItems());

        var listDeletedReinserted = listPostSpec!.concat([rItemFromGetAll]).sort((a, b) => {
            return a.id - b.id;
        });
        expect(listPreSpec!.length).toBe(listPostSpec!.length + 1);
    ...
    it('should get the item with a specified randomly selected existing id', async () => {
        ...
        let rItemFromGetById = await lastValueFrom(service.getArtItem(rItemFromGetAll.id));
